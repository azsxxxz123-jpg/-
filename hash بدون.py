def sha1(message):
    # تحويل النص إلى bytes إذا كان string
    if isinstance(message, str):
        message = message.encode('utf-8')
    
    # قيم ابتدائية (مطلوبة في خوارزمية SHA-1)
    h0 = 0x67452301
    h1 = 0xEFCDAB89
    h2 = 0x98BADCFE
    h3 = 0x10325476
    ...
    # حساب طول الرسالة وإضافة البادئة
    ml = len(message) * 8
    message += b'\x80'
    
    # إضافة الأصفار حتى يصبح الطول مناسباً
    while (len(message) % 64) != 56:
        message += b'\x00'
    
    # إضافة طول الرسالة الأصلي (64-bit)
    message += ml.to_bytes(8, byteorder='big')
    
    # معالجة كل قطعة (64-byte)
    for i in range(0, len(message), 64):
        chunk = message[i:i+64]
        w = [0] * 80
        
        # تقسيم القطعة إلى 16 كلمة (32-bit)
        for j in range(16):
            w[j] = int.from_bytes(chunk[j*4:j*4+4], byteorder='big')
        
        # توسيع الكلمات إلى 80 كلمة
        for j in range(16, 80):
            w[j] = _left_rotate(w[j-3] ^ w[j-8] ^ w[j-14] ^ w[j-16], 1)
        
        # تهيئة متغيرات الهاش لهذه القطعة
        a, b, c, d, e = h0, h1, h2, h3, h4
        
        # الحلقة الرئيسية لخوارزمية SHA-1
        for j in range(80):
            if j < 20:
                f = (b & c) | ((~b) & d)
                k = 0x5A827999
    ...
    # دمج القيم وإرجاع النتيجة
    return '%08x%08x%08x%08x%08x' % (h0, h1, h2, h3, h4)

def _left_rotate(n, b):
    # دوران البتات لليسار - دالة مساعدة
    return ((n << b) | (n >> (32 - b))) & 0xffffffff

def md5(message):
    # إذا كان النص string، نحوله إلى bytes
    if isinstance(message, str):
        message = message.encode('utf-8')
    
    # الثوابت المستخدمة في خوارزمية MD5
    ...
    for i in range(0, len(message), 64):
        ...
        # تهيئة المتغيرات لهذه القطعة
        a, b, c, d = a0, b0, c0, d0
        
        # الحلقة الرئيسية لخوارزمية MD5
        for j in range(64):
            if j < 16:
                f = (b & c) | ((~b) & d)
                g = j
...
def simple_hash(message, bits=32):
    ...
    # معالجة كل byte في الرسالة
    for byte in message:
        h = ((h << 5) + h) ^ byte
        h &= ((1 << bits) - 1)  # التأكد من أن الهاش لا يتجاوز الطول المطلوب
    
    # تحويل النتيجة إلى تنسيق hex
    hex_length = bits // 4
    format_str = '%0' + str(hex_length) + 'x'
    return format_str % h

if __name__ == "__main__":
    # اختبار الدوال على رسالة مثال
